...
async def merge_users(req: MergeRequest, admin_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # 3. Transfer Google Credentials if target doesn't have them
    if not target.google_id and source.google_id:
        target.google_id = source.google_id
        target.google_refresh_token = source.google_refresh_token
