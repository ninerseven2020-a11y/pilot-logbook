...
class CAD407Logbook:
    def __init__(self, user_id=None, pilot_name="L CHIANG"):
        ...
        self.COLUMN_MAP = {
            'DEP': ['DEP', 'DATE', 'DEPARTURE', 'FLIGHT DATE', 'UTC DATE', 'MONTH/DATE', 'MONTH', 'DAY'],
            'FLT_SN': ['FLT S/N', 'SERIAL', 'S/N', 'FLIGHT NO', 'FLT NO'],
            'AC_TYPE': ['AC TYPE', 'AIRCRAFT TYPE', 'TYPE', 'MODEL'],
            'AC_REG': ['AC REG', 'REGISTRATION', 'REG', 'TAIL NO', 'AIRCRAFT REG'],
            'CAPTAIN': ['CAPTAIN', 'PIC', 'COMMANDER', 'PILOT IN COMMAND', 'P1 NAME', 'PILOT-IN-COMMAND'],
            'COPILOT': ['COPILOT', 'FO', 'CO-PILOT', 'SIC', 'P2 NAME', 'CO-PILOT OR STUDENT'],
            'CAPACITY': ['OPERATING CAPACITY', 'CAPACITY', 'ROLE', 'FUNCTION', "HOLDER'S OPERATING CAPACITY"],
            'ROUTE': ['ROUTE', 'FROM-TO', 'FROM/TO', 'SECTOR', 'JOURNEY'],
            'DAY_P1': ['DAY P1', 'P1 DAY', 'DAY PIC', 'P1'],
            'DAY_P1US': ['DAY P1 (U/S)', 'DAY P1US', 'DAY P1 US', 'DAY PICUS', 'P1(U/S)'],
            'DAY_P2': ['DAY P2', 'P2 DAY', 'DAY SIC', 'DAY CO-PILOT', 'P2/P2X', 'P2'],
            'DAY_DUAL': ['DAY DUAL', 'DUAL DAY', 'INSTRUCTION DAY', 'P/UT', 'DUAL'],
            'NIGHT_P1': ['NIGHT P1', 'P1 NIGHT', 'NIGHT PIC', 'P1.1'],
            'NIGHT_P1US': ['NIGHT P1 (U/S)', 'NIGHT P1US', 'NIGHT P1 US', 'NIGHT PICUS', 'P1(U/S).1'],
            'NIGHT_P2': ['NIGHT P2', 'P2 NIGHT', 'NIGHT SIC', 'NIGHT CO-PILOT', 'P2/P2X.1', 'P2.1'],
            'NIGHT_DUAL': ['NIGHT DUAL', 'DUAL NIGHT', 'INSTRUCTION NIGHT', 'P/UT.1', 'DUAL.1'],
            'INSTRUMENT': ['INSTRUMENT', 'IFR', 'IF', 'INSTRUMENT TIME'],
    ...
    def normalize_history(self):
        """Ensures all entries in history have consistent keys for rendering."""
        for entry in self.history:
    ...
    def cad_round_up(self, value):
        ...
        if isinstance(value, time):
            ...
            return math.ceil(round(decimal_hours, 2) * 10) / 10.0
        ...
        try:
            return math.ceil(round(float(value), 2) * 10) / 10.0
    ...
    def parse_ias_row(self, row, operator="Default", label="Default", col_map=None):
        if not col_map:
            # Fallback for backward compatibility or when col_map isn't provided
            col_map = {k: v[0] for k, v in self.COLUMN_MAP.items()}

        def get_val(key, default=0.0):
            """Helper to handle NaN and mapped column names."""
            ...
            if pd.isna(val):
                # Small retry logic for trailing spaces just in case
        ...
        try:
            # Handle both string dates and pandas/excel Timestamp objects
            if isinstance(dep_val, datetime):
                ...
                try:
                    ...
                    # If it fails (common for dates like '28-Jan' without year), try appending current year
                    try:
